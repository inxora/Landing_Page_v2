/**
 * Libro de Reclamaciones (Ley N° 29571) — POST /api/libro-reclamaciones/
 * Misma forma que ecommerce-inxora.
 */
...
export const LibroReclamacionesService = {
  async crearReclamo(
    payload: LibroReclamacionesPayload
  ): Promise<LibroReclamacionesResponse> {
    const body = {
      ...
      acepta:
        payload.acepta === true ||
        String(payload.acepta || "").toLowerCase() === "si",
    };

    return api.post<LibroReclamacionesResponse>(
